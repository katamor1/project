...
if ! "$ROOT_DIR/build/tests_clang/test_system_shared" >> "$RESULT" 2>&1; then FAIL=1; fi
echo >> "$RESULT"
echo "===== test_ts_service =====" >> "$RESULT"
if ! "$ROOT_DIR/build/tests_clang/test_ts_service" >> "$RESULT" 2>&1; then FAIL=1; fi
echo >> "$RESULT"
echo "===== test_rt_control =====" >> "$RESULT"
if ! "$ROOT_DIR/build/tests_clang/test_rt_control" >> "$RESULT" 2>&1; then FAIL=1; fi
echo >> "$RESULT"
echo "===== test_nc_lookahead =====" >> "$RESULT"
if ! "$ROOT_DIR/build/tests_clang/test_nc_lookahead" >> "$RESULT" 2>&1; then FAIL=1; fi
echo >> "$RESULT"
echo "===== test_nc_interpolation =====" >> "$RESULT"
if ! "$ROOT_DIR/build/tests_clang/test_nc_interpolation" >> "$RESULT" 2>&1; then FAIL=1; fi
echo >> "$RESULT"
echo "===== test_nc_parser =====" >> "$RESULT"
if ! "$ROOT_DIR/build/tests_clang/test_nc_parser" >> "$RESULT" 2>&1; then FAIL=1; fi
echo >> "$RESULT"
...
if ! "$ROOT_DIR/build/tests_clang/test_nc_parser_feature_direct" >> "$RESULT" 2>&1; then FAIL=1; fi
echo >> "$RESULT"
echo "===== test_nc_codes_extended =====" >> "$RESULT"
if ! "$ROOT_DIR/build/tests_clang/test_nc_codes_extended" >> "$RESULT" 2>&1; then FAIL=1; fi
echo >> "$RESULT"
echo "===== test_nc_program_feature_status =====" >> "$RESULT"
if ! "$ROOT_DIR/build/tests_clang/test_nc_program_feature_status" >> "$RESULT" 2>&1; then FAIL=1; fi
echo >> "$RESULT"
echo "===== test_nc_coordinate_transform_direct =====" >> "$RESULT"
if ! "$ROOT_DIR/build/tests_clang/test_nc_coordinate_transform_direct" >> "$RESULT" 2>&1; then FAIL=1; fi
echo >> "$RESULT"
echo "===== test_nc_safety_motion_edges =====" >> "$RESULT"
if ! "$ROOT_DIR/build/tests_clang/test_nc_safety_motion_edges" >> "$RESULT" 2>&1; then FAIL=1; fi
echo >> "$RESULT"
echo "===== test_nc_reference_edges =====" >> "$RESULT"
if ! "$ROOT_DIR/build/tests_clang/test_nc_reference_edges" >> "$RESULT" 2>&1; then FAIL=1; fi
echo >> "$RESULT"
echo "===== test_nc_tool_management_timeout =====" >> "$RESULT"
if ! "$ROOT_DIR/build/tests_clang/test_nc_tool_management_timeout" >> "$RESULT" 2>&1; then FAIL=1; fi
echo >> "$RESULT"
echo "===== test_nc_precision_edges =====" >> "$RESULT"
if ! "$ROOT_DIR/build/tests_clang/test_nc_precision_edges" >> "$RESULT" 2>&1; then FAIL=1; fi
echo >> "$RESULT"
# Additional v7 unit tests
echo "===== test_nc_feed_profile_boundaries =====" >> "$RESULT"
if ! "$ROOT_DIR/build/tests_clang/test_nc_feed_profile_boundaries" >> "$RESULT" 2>&1; then FAIL=1; fi
echo >> "$RESULT"
echo "===== test_nc_cycle_segment_variants =====" >> "$RESULT"
if ! "$ROOT_DIR/build/tests_clang/test_nc_cycle_segment_variants" >> "$RESULT" 2>&1; then FAIL=1; fi
echo >> "$RESULT"
echo "===== test_nc_capability_history_wrap =====" >> "$RESULT"
if ! "$ROOT_DIR/build/tests_clang/test_nc_capability_history_wrap" >> "$RESULT" 2>&1; then FAIL=1; fi
echo >> "$RESULT"
echo "===== test_nc_interference_warning_edges =====" >> "$RESULT"
if ! "$ROOT_DIR/build/tests_clang/test_nc_interference_warning_edges" >> "$RESULT" 2>&1; then FAIL=1; fi
echo >> "$RESULT"
echo "===== test_nc_spindle_orientation_edges =====" >> "$RESULT"
if ! "$ROOT_DIR/build/tests_clang/test_nc_spindle_orientation_edges" >> "$RESULT" 2>&1; then FAIL=1; fi
echo >> "$RESULT"
# Additional v10 unit tests
echo "===== test_nc_coordinate_program_machine_edges =====" >> "$RESULT"
if ! "$ROOT_DIR/build/tests_clang/test_nc_coordinate_program_machine_edges" >> "$RESULT" 2>&1; then FAIL=1; fi
echo >> "$RESULT"
echo "===== test_nc_buffer_generation_validity_edges =====" >> "$RESULT"
if ! "$ROOT_DIR/build/tests_clang/test_nc_buffer_generation_validity_edges" >> "$RESULT" 2>&1; then FAIL=1; fi
echo >> "$RESULT"
echo "===== test_nc_axis_config_apply_edges =====" >> "$RESULT"
if ! "$ROOT_DIR/build/tests_clang/test_nc_axis_config_apply_edges" >> "$RESULT" 2>&1; then FAIL=1; fi
echo >> "$RESULT"
echo "===== test_nc_coordinate_transform_apply_edges =====" >> "$RESULT"
if ! "$ROOT_DIR/build/tests_clang/test_nc_coordinate_transform_apply_edges" >> "$RESULT" 2>&1; then FAIL=1; fi
echo >> "$RESULT"
echo "===== test_system_shared_event_log_more_edges =====" >> "$RESULT"
if ! "$ROOT_DIR/build/tests_clang/test_system_shared_event_log_more_edges" >> "$RESULT" 2>&1; then FAIL=1; fi
echo >> "$RESULT"
# Additional v9 unit tests
echo "===== test_control_api_remaining_setters =====" >> "$RESULT"
if ! "$ROOT_DIR/build/tests_clang/test_control_api_remaining_setters" >> "$RESULT" 2>&1; then FAIL=1; fi
echo >> "$RESULT"
echo "===== test_nc_program_request_edges =====" >> "$RESULT"
if ! "$ROOT_DIR/build/tests_clang/test_nc_program_request_edges" >> "$RESULT" 2>&1; then FAIL=1; fi
echo >> "$RESULT"
echo "===== test_rt_control_dispatch_edges =====" >> "$RESULT"
if ! "$ROOT_DIR/build/tests_clang/test_rt_control_dispatch_edges" >> "$RESULT" 2>&1; then FAIL=1; fi
echo >> "$RESULT"
echo "===== test_ts_service_log_and_slice_edges =====" >> "$RESULT"
if ! "$ROOT_DIR/build/tests_clang/test_ts_service_log_and_slice_edges" >> "$RESULT" 2>&1; then FAIL=1; fi
echo >> "$RESULT"
echo "===== test_nc_motion_filter_multiaxis_accel_edges =====" >> "$RESULT"
if ! "$ROOT_DIR/build/tests_clang/test_nc_motion_filter_multiaxis_accel_edges" >> "$RESULT" 2>&1; then FAIL=1; fi
echo >> "$RESULT"
...
do
  echo "===== $name =====" >> "$RESULT"
  if ! "$ROOT_DIR/build/tests_clang/$name" >> "$RESULT" 2>&1; then FAIL=1; fi
  echo >> "$RESULT"
done
