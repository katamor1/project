<!-- docs/claude-code/runtime-bridge/samples/email-notification-diff-dry-run.md -->
<!-- Shows a minimal bridge flow for a legacy diff request that returns a user-facing diff summary. -->
<!-- This exists so the team can verify diff dispatch, gate review, and Claude Code response shaping without starting a P-lane. -->
...
```json
{
  ...
  "requested_lane": "diff",
  "locale": "ja-JP",
  "session_context_ref": "thread://email-notify-diff"
}
```
...
```json
{
  ...
  "session_scope_ref": "BID-EMAIL-NOTIFY-001",
