<!-- .copilot/prompts/sdlc/ibmbob-sdlc-integration-reviewer.md -->
...
<!-- RELEVANT FILES: .copilot/schemas/integration-review-findings.schema.json, .copilot/schemas/artifact-context-packet.schema.json, .copilot/prompts/runtime/ibmbob-evidence-reviewer.md -->
# IBM Bob SDLC Integration Reviewer
...
- `frontend_contract` は `ibmbob-sdlc-frontend-draft-author` に戻します。
- `backend_contract` は `ibmbob-sdlc-backend-draft-author` に戻します。
- `sqlite_migration` は `ibmbob-sdlc-sqlite-draft-author` に戻します。
- `test_coverage` は `ibmbob-sdlc-test-draft-author` に戻します。
- `bundle_boundary` と `repo_mapping` は `ibmbob-sdlc-integration-bundle-planner` に戻します。
...
```json
{
  "prompt_id": "ibmbob-sdlc-integration-reviewer",
  "prompt_version": "1.0",
