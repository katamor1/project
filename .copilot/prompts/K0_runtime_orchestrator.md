...
<!-- This exists so the external runtime can route legacy KB tasks after the Claude Code entry bridge has already normalized the request. -->
<!-- RELEVANT FILES: .copilot/schemas/entry-dispatch-packet.schema.json, .copilot/routing/legacy-kb-query-matrix.yaml, docs/claude-code/runtime-bridge/dispatch-contract.md -->
...
## User
- 入力は `entry_dispatch_packet`, `runtime_request`, `run_state`, `session_scope_context` です。
...
- `required_inputs` は `entry_dispatch_packet`, `runtime_request`, `run_state`, `session_scope_context` です。
...
```json
{
  ...
  "required_inputs": ["entry_dispatch_packet", "runtime_request", "run_state", "session_scope_context"],
