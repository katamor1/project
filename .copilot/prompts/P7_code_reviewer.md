...
<!-- Defines the code review prompt for checking slice and test intent before repository handoff. -->
<!-- This exists so implementation prep returns a clear pass, revise, or block decision before bridge packaging. -->
<!-- RELEVANT FILES: .copilot/schemas/code-review-findings.schema.json, .copilot/routing/implementation-matrix.yaml, .copilot/prompts/P8_review_record.md -->
# P7 Code Reviewer
...
## User
- 入力は `fullstack_slice`, `test_pack`, `source_refs` です。
...
- `required_inputs` は `fullstack_slice`, `test_pack`, `source_refs` です。
...
```json
{
  ...
  "required_inputs": ["fullstack_slice", "test_pack", "source_refs"],
