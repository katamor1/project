...
<!-- RELEVANT FILES: .copilot/schemas/retrieval-plan.schema.json, .copilot/schemas/evidence-bundle.schema.json, .copilot/routing/runtime/ibmbob-runtime-query-flow.yaml -->
# IBM Bob Retrieval Planner
...
- diff 要求なら `ibmbob-legacy-diff-checker` に進めます。
- 通常生成なら `ibmbob-artifact-context-packer` に進めます。

## Assistant
- `artifact_type` は `retrieval_plan` 固定です。
- `required_inputs` は `artifact_request`, `identity_context`, `query_context` です。
- `next_agent` は `ibmbob-legacy-diff-checker` または `ibmbob-artifact-context-packer` です。
...
```json
{
  "prompt_id": "ibmbob-retrieval-planner",
  "prompt_version": "1.0",
